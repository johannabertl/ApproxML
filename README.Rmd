...
The algorithm is described in detail in [@BertlEtAl17]. 
...
Note that the fast version in the branch Rcpp uses C++ code via the interface Rcpp [@EdelbuettelFrancois11]. To install this, a C++ compiler is required. Details about Rcpp can be found at [www.rcpp.org](http://www.rcpp.org/) and [CRAN](https://cran.r-project.org/web/packages/Rcpp/index.html).


## R implementation (master branch)

Three versions of the approximate maximum likelihood algorithm are implemented: KDKW.FD (finite differences), KDKW.FD.CRN (finite differences with common random numbers) and KDKW.SP (simultaneous perturbations). The finite differences and simultaneous perturbations versions are described in detail [@BertlEtAl17] and the corresponding help pages. 
...
A faster version of the algorithm is implemented using Rcpp and RcppArmadillo. The R function KDKW_FD_Rcpp contains approximately equivalent functionality as KDKW.FD. 

A few simulation functions are included, but unlike KDKW.FD, where the simulation function is an R function, in KDKW_FD_Rcpp, the simulation function is implemented in Rcpp. 
...
The basic class `Model` is implemented in the file `Model.h`. It consists of a numeric vector parameters (the parameters to be estimated by the AML algorithm) and an optional list of fixed parameters. 
It contains two generic methods, Create_Model and simulate: 
