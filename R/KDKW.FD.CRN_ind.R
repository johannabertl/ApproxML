...
KDKW.FD.CRN_ind = function(s.obs, theta.0, simfun, rest=matrix(c(-Inf, Inf), ncol=2, nrow=length(theta.0), byrow=T),
  ...
  for (k in 2:K){
    ...
    for(j in 1:p){
        ...
        # for each beta:
        s.dim = ncol(s.minus)/2

        dens.minus.vec = numeric(s.dim)
        dens.plus.vec = numeric(s.dim)
        ...
        for(S in 1:s.dim){
          ...
          if (Hnum == "once"){
            if(k == 2){
              ...
              H.plus = Hfun(s.plus[,(S*2-1):(S*2)])
              H.minus = Hfun(s.minus[,(S*2-1):(S*2)])
            }
          } else {
          ...
          dens.plus.vec[S] = kde(s.plus[,(S*2-1):(S*2)], s.obs[(S*2-1):(S*2)], H.plus, kernel)
          dens.minus.vec[S] = kde(s.minus[,(S*2-1):(S*2)], s.obs[(S*2-1):(S*2)], H.minus, kernel)

        }

        dens.plus = prod(dens.plus.vec)
        dens.minus = prod(dens.minus.vec)
...
#' @describeIn KDKW.FD.CRN_ind The same function for a set of different starting values, use e. g. with apply or mclapply. It uses \code{try} catch errors.
#' @export

KDKW.FD.CRN_ind.theta.0 = function(theta.0, s.obs, simfun, rest=matrix(c(-Inf, Inf), ncol=2, nrow=length(theta.0), byrow=T),
  ce, gamma=1/6, C=0, a, alpha=1, A=0, K, nk,
  Hfun = bw.nrd0.mult, Hnum, kernel,
  lg=T,
  ...){

	try(KDKW.FD.CRN_ind(s.obs=s.obs, theta.0=theta.0, simfun=simfun, rest=rest,
...
#' @describeIn KDKW.FD.CRN_ind The same function for a set of different summary statistics, use e. g. with apply or mclapply. It uses \code{try} catch errors.
#' @export

KDKW.FD.CRN_ind.s.obs = function(s.obs, theta.0, simfun, rest=matrix(c(-Inf, Inf), ncol=2, nrow=length(theta.0), byrow=T),
  ce, gamma=1/6, C=0, a, alpha=1, A=0, K, nk,
  Hfun = bw.nrd0.mult, Hnum, kernel,
  lg=T,
  ...){
